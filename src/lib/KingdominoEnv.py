...
action_space = Tuple(choose_space,play_space)
...
class KingdominoEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # The action should include both the domino index and the coordinates
        domino_index, placement = action

        # Pick the domino from the shop
        chosen_domino = self.pick_domino(domino_index)

        # Play the domino
        self.play(chosen_domino, placement)

        # Update the state, calculate reward, check if done, etc.
        # ...
        pass
